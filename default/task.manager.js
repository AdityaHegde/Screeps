var TASKS = [{
    name : "harvest",
    api : require("task.harvest"),
}, {
    name : "upgrade",
    api : require("task.upgrade"),
}, {
    name : "build",
    api : require("task.build"),
}],
TASK_NAME_TO_IDX = {
    "harvest" : 0,
    "upgrade" : 1,
...
var taskManager = {
    ...
    initSpawn : function(spawn) {
        ...
        TASKS.forEach(function(task) {
            spawn.memory.taskManager.tasksInfo[task.name] = {
                targets : [],
                hasTarget : false,
                currentTarget : null,
                workers : {},
            ...
            task.api.initSpawn(spawn, spawn.memory.taskManager.tasksInfo[task.name]);
        });
    },
    ...
    tick : function(spawn) {
        ...
        TASKS.forEach(function(task) {
            ...
            if (taskInfo.hasTarget && this.taskIsNotFull(spawn, taskInfo)) {
                spawn.memory.taskManager.freeTasks[task.name] = 1;
                spawn.memory.taskManager.hasFreeTasks = true;
            }
        }.bind(this));

        //console.log(spawn.memory.taskManager.hasFreeTasks, Object.keys(spawn.memory.taskManager.freeTasks).join(","), spawn.memory.taskManager.validTasks);
    },
